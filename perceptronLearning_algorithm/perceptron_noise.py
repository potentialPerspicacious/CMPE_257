...
y_noise1 = (y_train[-100:]) * -1
y_noise2 = (y_train[:-100])
y_noise = np.concatenate((y_noise2, y_noise1))

x_cord = x_train[1, :]
y_cord = x_train[2, :]
colors = ['b' if y > 0 else 'r' for y in y_noise]

plt.scatter(x_cord, y_cord, c=colors)
x_cord_hypo = np.array([-10, 10])

h_final = percep_algo(h_initial, x_train, y_noise, plot_iteration)
